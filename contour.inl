...
//    }
        ...
        for (int i = 1; i<Nrow-1; ++i)
        {
            for (int j = 1; j<Ncol-1; ++j)
            {
                if(image_cp (i,j)*image_cp(i+1,j)*image_cp(i,j+1)*image_cp(i-1,j)*image_cp(i,j-1)*image_cp(i+1,j+1)*image_cp(i-1,j-1)*image_cp(i-1,j+1)*image_cp(i+1,j-1) != 0)
                {
                    if( image_cp(i+1,j) < image_cp(i,j)-N || image_cp(i,j+1) < image_cp(i,j)-N || image_cp(i+1,j+1) < image_cp(i,j)-N || image_cp(i-1,j+1) < image_cp(i,j)-N|| image_cp(i+1,j-1) < image_cp(i,j)-N || image_cp(i-1,j-1) < image_cp(i,j)-N || image_cp(i-1,j) < image_cp(i,j)-N || image_cp(i,j-1) < image_cp(i,j)-N)
                    {
                        if( image_cp(i,j) > 240 )
                            imager(i,j) = max_col;
                    }
